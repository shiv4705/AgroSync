...
export default function LoginForm() {
  ...
  const [currentTime, setCurrentTime] = useState("2025-03-19 20:04:42");
  const [currentUser, setCurrentUser] = useState("Dishang18");
  ...
  // Clear debug info when component unmounts
  useEffect(() => {
    return () => setDebugInfo(null);
  }, []);
  ...
  const logUserActivity = async (userId, action) => {
    try {
      console.log(`[${currentTime}] User ${userId || currentUser}: ${action}`);
      // You could implement an actual API call to log activity to your backend
    } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      console.log(`[${currentTime}] ${currentUser} attempting login with email: ${email}`);
      ...
      // Log user role for debugging - use optional chaining
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      console.log(`[${currentTime}] ${currentUser} attempting Google Sign-In`);
      ...
      // Get user role from the nested user object
      const userRole = data.user?.role;
    ...
    <div className="min-h-screen bg-[#1a332e]">
      ...
      <div className="min-h-screen bg-[#003333] flex flex-col items-center justify-center p-6 relative overflow-hidden">
        ...
        {process.env.NODE_ENV !== 'production' && debugInfo && (
          <div className="fixed bottom-4 right-4 bg-gray-900 text-white p-4 rounded-md shadow-lg text-xs max-w-md overflow-auto max-h-60 z-50">
        ...
        <div className="w-full max-w-md relative z-10">
          ...
          <div className="mt-4 text-center text-gray-500 text-xs">
            Last updated: {currentTime} • {currentUser}
          </div>
        </div>
      </div>
    </div>
